...
public final class ConsulServiceMgr {
	...
	private static NewService createService(final Integer port, final String serviceId, final String serviceNm,
		...
		log.info("host:[" + host + "]port:[" + port + "],serviceId[" + serviceId + "],serviceNm:[" + serviceNm
	...
	public static List<ServiceInfo> getHealthlyServicesByNm(String serviceNm) {
		return getHealthlyServicesByNm(serviceNm, null);
	}

	public static List<ServiceInfo> getHealthlyServicesByNm(String serviceNm, String token) {
		ConsulClient client = ConsulClientMgr.getConsulClient();
		...
		for (HealthService hs : listService) {
	...
	public static ServiceInfo getHealthlyServiceByNm(String serviceNm, String token) {
		List<ServiceInfo> listService = getHealthlyServicesByNm(serviceNm);
		Random r = new Random();
		int idx = r.nextInt(listService.size());
		ServiceInfo info = listService.get(idx);
		log.debug("getService from consul:" + info);
		return info;
	}
